...
void color_bounceFADE() {                    //-m6-BOUNCE COLOR (SIMPLE MULTI-LED FADE)
  ...
  for (int i = 0; i < LED_COUNT; i++ ) {
    if (i == idex) {
      leds[i] = CHSV(thishue, 255, thissat);
    }
    else if (i == iL1) {
      leds[i] = CHSV(thishue, 150, thissat);
    }
    else if (i == iL2) {
      leds[i] = CHSV(thishue, 80, thissat);
    }
    else if (i == iL3) {
      leds[i] = CHSV(thishue, 20, thissat);
    }
    else if (i == iR1) {
      leds[i] = CHSV(thishue, 150, thissat);
    }
...
void flameBack() {                                    //-m22-FLAMEISH EFFECT
  int idelay = random(0, 25);
  float hmin = 0.1; float hmax = 45.0;
  float hdif = hmax - hmin;
  int randtemp = random(0, 3);
